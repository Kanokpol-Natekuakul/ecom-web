  ...
  return (
    ...
    <div className='container mx-auto p-4 bg-white shadow-md'>
      <form onSubmit={handleSubmit}>
        ...
        <button className='bg-blue-500 p-2 rounded-md shadow-md hover:scalr-105 hover:-translate-y-1 hover:duration-200'>เพิ่มสินค้า</button>
        <hr />
        <br />
              ...
              products.map((item, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>
                      {
                        item.images.length > 0 ? <img className='w-24 h-24 rounded-lg shadow-md' src={item.images[0].url} alt="" /> : <div className='w-24 h-24 bg-gray-200 rounded-md flex items-center justify-center shadow-sm'>No Image</div>
                      }
                    </td>
                    <td>{item.title}</td>
                    ...
                    <td className='flex gap-2'>
                      <p className='bg-yellow-500 rounded-md p-1 shadow-md hover:scalr-105 hover:-translate-y-1 hover:duration-200'><Link to={'/admin/product/' + item.id}><Pencil />
                      </Link></p>
                      <p className='bg-red-500 rounded-md p-1 shadow-md hover:scalr-105 hover:-translate-y-1 hover:duration-200' onClick={() => handleDelete(item.id)}><Delete /></p>
                    </td>
                  </tr>
                )
              })
